...
export default function EmploymentTypeTable() {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down("sm"));

  const [rows, setRows] = useState<EmploymentTypeRow[]>([]);
  ...
  const handleCancelClick = (id: number) => {
    setRowModesModel((prev) => ({
      ...prev,
      [id]: {
        mode: "view" as unknown as GridRowModes,
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        const isEditing =
          rowModesModel[params.row.id]?.mode ===
          ("edit" as unknown as GridRowModes);
  ...
  const handleDelete = async (id: number) => {
    try {
      await api.delete(`/employment-type/${id}`);
      fetchEmploymentTypes();
  ...
  const handleUpdate = async (id: number, name: string) => {
    try {
      await api.put(`/employment-type/${id}`, { name });
  ...
  const onSubmit = async (data: EmploymentTypeForm) => {
    try {
      await api.post("/create-employment-type", { name: data.name });
      fetchEmploymentTypes();
      reset();
      handleClose();
        ...
        >
          ...
          >
            <Typography sx={{ fontWeight: 700, fontSize: { xs: 14, sm: 18 }, color: NAVY_TEXT, whiteSpace: "nowrap" }}>
              All Employment Types ({filteredRows.length})
            </Typography>
            <Button
              variant="contained"
              startIcon={<AddIcon />}
            ...
            >
              {isMobile ? "Add" : "Add Employment Type"}
            </Button>
          </Box>
            ...
            <Box sx={{ minWidth: { xs: 320, sm: "100%" } }}>
              <DataGrid
                  ...
                  noRowsOverlay: () => (
                    ...
                    >
                      <Typography sx={{ color: "#94a3b8", fontSize: 14, textAlign: "center", px: 2 }}>
                        No employment types found. Create your first employment type.
                      </Typography>
                    </Box>
                  ),
                }}
                sx={{
                  ...
                  "& .MuiDataGrid-cell--editing": { bgcolor: "transparent !important", boxShadow: "none !important" },
                  "& .MuiDataGrid-cell--editing .MuiInputBase-root": { fontSize: 13, color: "#334155" },
                  "& .MuiDataGrid-cell": {
      ...
      >
        <DialogTitle sx={{ pb: 0.5, pt: 3, px: 3 }}>
          ...
          <Typography sx={{ color: "#94a3b8", fontSize: 13, mt: 0.3 }}>
            Enter the employment type below.
          </Typography>
        </DialogTitle>
        ...
        <DialogContent sx={{ px: 3, py: 2 }}>
          <form onSubmit={handleSubmit(onSubmit)} id="employment-type-form">
            <TextField
              autoFocus
              required
